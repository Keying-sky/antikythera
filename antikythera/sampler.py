...
class AntikytheraHMC:
    ...
    def build_model(self, is_aniso: bool = True, 
                   N_prior: Tuple[float, float] = (330, 370),
                   r_prior: Tuple[float, float] = (60, 90),
                   sigma_prior: Tuple[float, float] = (0.02, 0.2),
        ...
        with pm.Model() as model:
            ...
            if is_aniso:
                pm.Potential(
                    "init_term",
                    -self.n_holes * pm.math.log(2 * np.pi * sigma_r * sigma_t)
                )
            else:
                pm.Potential(
                    "init_term",
                    -self.n_holes * pm.math.log(2 * np.pi * sigma**2)
                )
...
def position_samples(trace, is_aniso, hole_idx, section_idx, nsamples=100):                           
    """
    Generate posterior predictive samples for a specific hole.
    
    Args:
        trace: Posterior trace from HMC sampler
        is_aniso: Whether the model is anisotropic or isotropic
    ...
    Returns: Array of predicted hole positions (x, y)
    """
    # get all parameter samples from the trace
    N = trace.posterior["N"].values.flatten()
    r = trace.posterior["r"].values.flatten()
    x0 = trace.posterior[f"x0_{section_idx}"].values.flatten()
    y0 = trace.posterior[f"y0_{section_idx}"].values.flatten()
    alpha = trace.posterior[f"alpha_{section_idx}"].values.flatten()
    
    if is_aniso:
        sigma_r = trace.posterior["sigma_r"].values.flatten()
        sigma_t = trace.posterior["sigma_t"].values.flatten()
    else:
        sigma = trace.posterior["sigma"].values.flatten()
    
    idx = np.random.choice(len(N), nsamples, replace=True)
    
    predictions = []
    for i in idx:
        phi = 2 * np.pi * (hole_idx - 1) / N[i] + alpha[i]
        
        # predicted position without error
        x_pred = x0[i] + r[i] * np.cos(phi)
        y_pred = y0[i] + r[i] * np.sin(phi)
        
        # generate errors from the error model
        if is_aniso:
            e_r = np.random.normal(0, sigma_r[i])
            e_t = np.random.normal(0, sigma_t[i])
        else:
            e_r = np.random.normal(0, sigma[i])
            e_t = np.random.normal(0, sigma[i])
        ...
        predictions.append([x_pred_with_error, y_pred_with_error])
